...
def main():
    ...
    if os.path.exists("./models/zsl_awa1_attribute_network_v33.pkl"):
        attribute_network.load_state_dict(torch.load("./models/zsl_awa1_attribute_network_v33.pkl",map_location="cpu"))
        print("load attribute network success")
    if os.path.exists("./models/zsl_awa1_relation_network_v33.pkl"):
        relation_network.load_state_dict(torch.load("./models/zsl_awa1_relation_network_v33.pkl",map_location="cpu"))
    ...
    for episode in range(EPISODE):
        ...
        re_relations = torch.add(torch.div(relations, -1), 1).cuda(GPU)

        for index in range(relations.shape[0]):
            re_relations[int(index)][int(re_batch_labels[int(index)])] = relations[int(index)][
                int(re_batch_labels[int(index)])]
        ...
        #    GPU)
        loss = torch.sum(torch.mul(I, relations))
        ...
        if (episode + 1) % 2000 == 0:
            ...
            if H > high_H:
                ...
                torch.save(attribute_network.state_dict(),
                           "./models/zsl_awa1_attribute_network_cpdn.pkl")
                torch.save(relation_network.state_dict(),
                           "./models/zsl_awa1_relation_network_cpdn.pkl")
                #
                print("save networks for episode:", episode)

                # save networks

            if gzsl_unseen_accuracy > unseen:
                unseen = gzsl_unseen_accuracy
            if gzsl_seen_accuracy > seen:
                seen = gzsl_seen_accuracy

    print("final accuracy_score: ", last_accuracy)
    print('gzsl: seen=%.4f, unseen=%.4f, h=%.4f' % (H_seen, H_unseen, high_H))
    print("seen: ", seen)
    print("unseen: ", unseen)
