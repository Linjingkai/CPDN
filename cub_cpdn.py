...
def main():
    ...
    if os.path.exists("./models/zsl_cub_attribute_network_v35.pkl"):
        attribute_network.load_state_dict(torch.load("./models/zsl_cub_attribute_network_v35.pkl",map_location="cpu"))
        print("load attribute network success")
    if os.path.exists("./models/zsl_cub_relation_network_v35.pkl"):
        relation_network.load_state_dict(torch.load("./models/zsl_cub_relation_network_v35.pkl",map_location="cpu"))
        print("load relation network success")

    print("training...")
    last_accuracy = 0.0
    last_accuracy_unseen = 0.0
    last_accuracy_seen = 0.0
    last_accuracy_H = 0.0
    H_seen =0.0
    H_unseen =0.0
    ...
    for episode in range(EPISODE):
        ...
        equal_array = []
        #
        for i in batch_labels.numpy():
            for j in sample_labels:
                equal_array.append(isequal(i, j))

        I = Variable(torch.FloatTensor(equal_array).view(-1, class_num)).cuda(GPU)

        sample_features_1 = sample_features.unsqueeze(0).repeat(class_num, 1, 1).view(-1, 2048)
        sample_features_2 = sample_features.unsqueeze(0).repeat(class_num, 1, 1)
        sample_features_2 = torch.transpose(sample_features_2, 0, 1)
        sample_features_2 = sample_features_2.contiguous().view(-1, 2048)
        # cos_distance = cos(sample_features_1, sample_features_2).view(class_num, class_num)
        relation_pair = torch.cat((sample_features_1, sample_features_2), 1).view(-1, 4096)
        relation = relation_network(relation_pair).view(-1, class_num)

        # loss
        #mse = nn.MSELoss().cuda(GPU)
        #one_hot_labels = Variable(torch.zeros(BATCH_SIZE, class_num).scatter_(1, re_batch_labels.view(-1, 1), 1)).cuda(
        #    GPU)
        loss = torch.sum(torch.mul(I,relations)) + torch.sum(relation)-torch.trace(relation)
#
        # update
        ...
        if (episode) % 2000 == 0:
            ...
            if gzsl_seen_accuracy > last_accuracy_seen:
                last_accuracy_seen = gzsl_seen_accuracy
            if gzsl_unseen_accuracy > last_accuracy_unseen:
                last_accuracy_unseen = gzsl_unseen_accuracy
            if H > last_accuracy_H:
                last_accuracy_H = H
                H_unseen = gzsl_unseen_accuracy
                H_seen = gzsl_seen_accuracy
                # save networks
                torch.save(attribute_network.state_dict(),
                           "./models/zsl_cub_attribute_network_cpdn.pkl")
                torch.save(relation_network.state_dict(), "./models/zsl_cub_relation_network_cpdn.pkl")
                #

                print("save networks for episode:", episode)

    print("final accuracy_score :", last_accuracy)
    print("final gzsl_seen_accuracy_score :", last_accuracy_seen)
    print("final gzsl_uneen_accuracy_score :", last_accuracy_unseen)
    print('gzsl: seen=%.4f, unseen=%.4f, h=%.4f' % (H_seen, H_unseen, last_accuracy_H))
